...
var ayChatContainer = {
  ...
  ayReloadConversationHistory: function() {
    ...
    if (typeof(selectedDiv) !== 'undefined' && selectedDiv.length > 0) {
      $('#ayHistoryListView')[0].scrollTop = selectedDiv[0].offsetTop-150;
    }
  },
  ...
  ayGenerateHistoricalDiv: function(ayHistory) {
    ...
    ayHDiv = ayHDiv.replace(/{CONVERSATION_ID}/g, "'" + ayHistory.key + "'");
  ...
  ayRenderHistoryView: function() {
    ...
    if (this.ayIsConsumer) {
      cDiv = cDiv.replace(/{FEEDBACK_LINK}/g, 'https://www.surveygizmo.com/s3/4075245/Consumer-Feedback-AskYellow-MyYellow');
      ...
      cDiv = cDiv.replace(/{FEEDBACK_LINK}/g, 'https://www.surveygizmo.com/s3/4075270/Business-Feedback-AskYellow-MyYellow');
      $('#' + this.ayParentDiv).append(cDiv);
    }
    ...
    if (this.ayConversationId !== null) {
      if (this.ayChatHistory !== null) {
        ...
        if (this.ayIsValid(history)) {
          ...
          $('#ayMobileMsg').empty();
          var mobileElement = document.getElementById("ayMobileMsg");
          var msg = "We’ve sent an email to you at ";
          msg = msg + (this.ayIsConsumer ? this.ayCId.replace(/__dot__/g,'.') : history.customer.businessEmail);
          msg = msg + ". You can use the link in the email to get back to this conversation with {BUSINESS_NAME} at anytime.".replace('{BUSINESS_NAME}', this.ayIsConsumer ? history.customer.businessName : history.nickname);
          mobileElement.appendChild(document.createTextNode(msg));
  ...
  ayToggleMobileInfo: function() {
    if ($('#ay-message-open').hasClass('ay-message-active')) {
      $('#ay-message-open').removeClass('ay-message-active');
    } else {
      $('#ay-message-open').addClass('ay-message-active');
    }
  },

  ayOnClickBack: function() {
    $('#ay-message-open').removeClass('ay-message-active');

    var historyDiv = $('#ayHistoryListView');
    if (typeof(historyDiv) !== 'undefined' && historyDiv.length > 0) {
      if (navigator.userAgent.match(/(iPod|iPhone|iPad|Android)/)) {
        window.scrollTo(0,0);
      } else {
        historyDiv[0].scrollTop = 0;
      }
    }
  },
  ...
  ayContainerDiv: '<div class="ay-search-clamp"> <div class="ay-messages ay-conversation-selected"> <div id="ayMessagesContainer"> <div id="ayUserHeader" class="ay-header"> </div> <div id="ayHistoryContainer" class="ay-conversation-history"> <div class="ay-sub-headers"><div id="ayMobileUserHeader" class="ay-sub-header ay-users-messages ay-small-screen">{PLACEHOLDER}</div> <div id="ayHistoryHeader" class="ay-sub-header"> My message history </div></div><div id="ayHistoryListView" class="ay-conversations"></div><div class="ay-mobile-feedback">If you’d like to <a href="{FEEDBACK_LINK}" target="_blank">provide us with feedback</a> on the new messaging feature, we’d love to hear from you.</div></div><div id="ayConversationContainer" class="ay-active-conversation"> <div class="ay-sub-headers"> <div id="ayBackLink" class="ay-sub-header ay-my-message-history ay-small-screen" onclick="ayChatContainer.ayOnClickBack()"> &lt; My message history </div> <div class="ay-info"><div id="infoIconContainer" class="ay-info-icon-container" onclick="ayChatContainer.ayToggleMobileInfo()"><div id="infoIcon" class="ay-info-icon"></div></div><div id="ay-message-open" class="ay-message-open"><div id="ayMobileMsg" class="ay-mobile-weve-sent-an-email">{FOTTER_MSG}</div><div class="ay-close-btn" onclick="ayChatContainer.ayToggleMobileInfo()">Close</div></div></div> <div id="ayConversationHeader" class="ay-sub-header"> My messages with Jim’s Mowing </div></div><div class="ay-bubbles-container-header"></div><div id="ayBubblesContainer" class="ay-bubbles-container"><ul id="ayConversationListView" class="ay-bubbles-table"></ul></div> <div class="ay-bubbles-container-footer ay-small-screen"></div> <div class="ay-message-input-area"> <textarea id="ayTextArea" class="ay-message" onkeydown="if(event.keyCode == 13) {ayChatContainer.ayOnClickSend(); return false;}" placeholder="Tap here to message Jim’s Mowing"></textarea> <div id="aySubmitButton" class="ay-submit-message" onclick="ayChatContainer.ayOnClickSend()"></div></div><div id="ayFooterMsg" class="ay-weve-sent-an-email">{FOTTER_MSG}</div></div><div class="ay-feedback">If you’d like to <a href="{FEEDBACK_LINK}" target="_blank">provide us with feedback</a> on the new messaging feature, we’d love to hear from you.</div></div></div></div>',
